...
class ProxyService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        thread(name = "ProxyInitThread") {
            startBinary()
            Thread.sleep(4000) // ждем, пока vkturn стартует
            startWireGuard()
        }
    ...
    private fun startWireGuard() {
        ...
        try {
            addLog("Подготовка WireGuard через vkturn relay...")

            // Endpoint теперь локальный vkturn
            val configText = """
                [Interface]
                PrivateKey = $privKey
                Address = $localIp
            ...
            addLog("WireGuard теперь полностью через vkturn relay")
        } catch (e: Exception) {
